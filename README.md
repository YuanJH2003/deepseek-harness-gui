...
- **点了图标第一次没反应，隔几秒再点才行**：这不是“点太快”，而是刚关掉的窗口所对应的服务还在收尾（优雅停服最多几秒，之后才释放单写者锁）。此窗口期内的点击会被“单写者锁”正确拦下（防止双写），但控制台被隐藏、看不到提示，看起来像“没反应”。新版网关发现“有持锁者但暂时不可达”时会**原地等待最多 8 秒**：旧服务一退出就自动接管启动，后面只需点一次；若 8 秒后仍不可达（真正残留的进程），再运行 `dsh-gui-stop.cmd` 清理即可。
