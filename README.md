...
- **任务栏图标**：应用窗口以 Edge 启动但带专属应用身份（`--app-user-model-id=DeepSeekHarnessGUI`），在任务栏中作为**独立应用**显示（不会与浏览器 Edge 归为同一组）；同时网关会运行 `dsh-gui-taskbar-icon.ps1` 把窗口图标替换为**深色鲸鱼**：跨进程向窗口发送 `WM_SETICON`（大/小图标）并改写窗口类图标，再发 `SWP_FRAMECHANGED` 强制重绘——这套组合覆盖 Win10/11 任务栏按钮图标的读取路径；脚本按 `--user-data-dir` 特征匹配窗口，无论 Edge 把窗口挂在哪个进程都能命中（失败时静默降级为 Edge 图标）。`dsh-gui.cmd --make-shortcut` 会为快捷方式打上同样的身份。普通 Edge 浏览窗口不受影响。若图标缓存未刷新，桌面图标可重启资源管理器或运行 `ie4uinit.exe -show` 恢复。
