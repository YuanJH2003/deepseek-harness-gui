...
export function webLockPath(): string {
  return dshHomePath(WEB_LOCK_NAME)
}

function readWebLock(path: string): WebLockRecord | undefined {
  try {
    const parsed = JSON.parse(readFileSync(path, 'utf8')) as Partial<WebLockRecord>
...
function holderLocation(record: WebLockRecord): string | undefined {
  if (record.url !== undefined) return record.url
  if (record.port !== undefined && record.port > 0) return `http://127.0.0.1:${String(record.port)}`
  return undefined
}

/** Refusal message naming the live holder and its location. */
function refusalMessage(record: WebLockRecord): string {
 ...
 */
export function guardWebServer(port: number, force = false): { status: 'ok' } | { status: 'refused'; message: string } {
...
export function releaseWebLock(path: string = webLockPath()): void {
  const record = readWebLock(path)
  if (record !== undefined && record.pid === process.pid) rmSync(path, { force: true })
}
 ...
 */
export function setWebLockUrl(
  url: string,
  port: number,
  ownerPid: number,
  path: string = webLockPath(),
): void {
  const record = readWebLock(path)
 ...
 */
export function liveWebLockHolder(path: string = webLockPath()): WebLockRecord | undefined {
  const record = readWebLock(path)
 ...
 */
export function portFromArgs(args: readonly string[], fallback = 3080): number {
